...
Extract every word in the text that is genuinely useful to learn, up to a maximum of 25 words (skip trivial words like articles, pronouns, and common function words). If the text only contains a handful of useful words, return just those — don't invent or pad with words that aren't naturally present in the text. For each word, also give its own CEFR difficulty level (A1-C2) so learners can gauge how hard it is. Keep every field brief — this is a quick-reference flashcard, not an essay. For each word, produce a short example sentence in German, ideally adapted from the source text itself.
